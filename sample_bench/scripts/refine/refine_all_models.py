...
if __name__ == "__main__":
    ...
    for cond in range(J):
        for col_type in ["r_free", "r_work", "xray"]:
            refined_log_df["{}_{}".format(col_type, cond)] = np.nan
    ...
    for i in range(len(pdb_files)-1):
        ...
        for cond in range(J):
            occs = [float(log_df.iloc[i]["w_{}_{}".format(state, cond)]) for state in range(N)]
            ...
            refined_log_df.loc[i, "xray_{}".format(cond)] = score_dict["ml"]
            refined_log_df.loc[i, "r_free_{}".format(cond)] = score_dict["r_free"]
            refined_log_df.loc[i, "r_work_{}".format(cond)] = score_dict["r_work"]
            refined_log_df.loc[i, "rmsd_{}".format(cond)] = score_dict["rmsd_avg"]
            refined_log_df.loc[i, "ff".format(cond)] = score_dict["ff"]
            print(score_dict)
